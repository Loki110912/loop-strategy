...
// Die leichte KI trifft bewusst keine Bewertung: Sie spielt regelkonform, aber unvorhersehbar.
export function chooseEasyMove(board, player, legalMoves) {
  const index = Math.floor(Math.random() * legalMoves.length);
  return legalMoves[index];
}

export function chooseEasyDirection() {
  return Math.random() < 0.5 ? DIRECTIONS.CLOCKWISE : DIRECTIONS.COUNTERCLOCKWISE;
}
