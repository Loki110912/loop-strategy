...
export const GAME_MODE_DETAILS = {
  [GAME_MODES.CLASSIC]: {
    label: "Klassisch",
    description: "Nimm dem Gegner alle Startmöglichkeiten.",
  },
  [GAME_MODES.POINT_HUNT]: {
    label: "Punktejagd",
    description: "Stehle zuerst 50 Punkte.",
  },
  [GAME_MODES.QUICK]: {
    label: "Schnellspiel",
    description: "Leere die gegnerische Frontreihe.",
  },
};

export function getDifficultyLabel(difficulty) {
  return difficulty === DIFFICULTIES.EASY ? "Leicht" : "Schwer";
}
...
export function isFrontRowEmpty(board, player) {
  return TRACKS[player]
    .filter((cell) => isFrontRow(player, cell))
    .every((cell) => board[cell.row][cell.col] === 0);
}
...
// Ermittelt eine Strecke für die Vorschau, ohne Spielfeld oder Zugzustand zu verändern.
export function getSowingPath(player, startCell, direction, steps) {
  const path = [];
  let currentCell = startCell;

  for (let step = 0; step < steps; step += 1) {
    currentCell = getNextCell(player, currentCell, direction);
